<div class="space-y-6" x-data="{
    ...
    <div
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
            <div>
                <h3 class="text-lg font-bold text-gray-900 dark:text-white">Daftar Departemen</h3>
                <p class="text-xs text-gray-500 dark:text-slate-400">Kelola informasi data departemen terdaftar.</p>
            </div>
            <button @click="showCreateModal = true"
                class="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white shadow-md shadow-emerald-600/20 dark:bg-[#93F514] dark:hover:bg-[#82dc12] dark:text-black dark:shadow-[#93F514]/20 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 shrink-0 self-start sm:self-auto active:scale-95 cursor-pointer">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <!-- Filter Toolbar -->
        <div
            class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 pt-4 border-t border-gray-100 dark:border-slate-800">
            <!-- Search -->
            ...
            @if ($search || $companyFilter)
                <div class="sm:col-span-2 lg:col-span-3 flex justify-end">
                    <button wire:click="resetFilters"
                        class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-lg transition-colors">
    ...
    <div
        ...
        <div wire:loading wire:target="search, companyFilter, previousPage, nextPage, gotoPage"
                        ...
                        <tr class="hover:bg-gray-50/80 dark:hover:bg-slate-800/40 transition-colors">
                            <td class="px-6 py-4 font-bold text-gray-900 dark:text-white">
                                <div class="flex items-center gap-2.5">
                                    <div
                                        class="w-8 h-8 rounded-lg bg-indigo-50 dark:bg-indigo-950/50 border border-indigo-200 dark:border-indigo-800 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-semibold shrink-0">
                            ...
                            <td class="px-6 py-4">
                                @if ($department->company)
                                    <span
                                        ...
                                        <svg class="w-3.5 h-3.5 text-blue-500 dark:text-sky-400 shrink-0" fill="none" viewBox="0 0 24 24"
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-gray-400 dark:text-slate-500">
                                <div class="flex flex-col items-center justify-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Belum ada data departemen</span>
                                    <span class="text-xs">Klik tombol "Tambah Departemen" untuk menambahkan data
                                        baru.</span>
                                </div>
                            </td>
                        </tr>
        ...
        @if ($departments->hasPages() || $perPage != 10)
            <div
    ...
    <div x-show="showCreateModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title"
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="showCreateModal" x-transition:enter="ease-out duration-300"
                ...
                x-transition:leave-end="opacity-0" @click="showCreateModal = false"
                class="fixed inset-0 transition-opacity bg-gray-900/60 dark:bg-black/70 backdrop-blur-sm"></div>
            ...
            <div x-show="showCreateModal" x-transition:enter="ease-out duration-300"
                ...
                class="inline-block align-bottom bg-white dark:bg-slate-900 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full border border-gray-200 dark:border-slate-800">
                ...
                <div class="p-6">
                    ...
                    <form action="{{ route('admin.department.store') }}" method="POST"
                        @submit="isSubmittingCreate = true" class="mt-4 space-y-4">
                        @csrf

                        <!-- Company Selection -->
                        <div>
                            <label for="company_id"
                                class="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1">
                                Perusahaan Naungan <span class="text-rose-500">*</span>
                            </label>
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name') }}"
                                required placeholder="Contoh: Engineering / Human Resources"
                                class="w-full px-3 py-2 text-xs rounded-xl bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-800 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition">
                            @error('name')
                                <p class="mt-1 text-[11px] text-rose-500">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3"
                                placeholder="Penjelasan mengenai tugas atau divisi ini..."
                                class="w-full px-3 py-2 text-xs rounded-xl bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-800 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-[11px] text-rose-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <!-- Modal Actions -->
                        <div
                            class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100 dark:border-slate-800">
                            <button type="button" @click="showCreateModal = false"
                                class="px-4 py-2 text-xs font-medium text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-xl transition">
                                Batal
                            </button>
                            <button type="submit" :disabled="isSubmittingCreate"
                                ...
                                <svg x-show="isSubmittingCreate"
                                    class="animate-spin w-3.5 h-3.5 text-white dark:text-black" fill="none"
                                    viewBox="0 0 24 24">
    ...
    <div x-show="showEditModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title-edit"
        role="dialog" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <!-- Background Overlay -->
            ...
            <div x-show="showEditModal" x-transition:enter="ease-out duration-300"
                ...
                <div class="p-6">
                    ...
                    <form :action="'/admin/departments/' + editData.id" method="POST"
                        @submit="isSubmittingEdit = true" class="mt-4 space-y-4">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <select name="company_id" id="edit_company_id" x-model="editData.company_id"
                                class="w-full px-3 py-2 text-xs rounded-xl bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-800 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition">
                                <option value="">-- Pilih Perusahaan --</option>
                                @foreach ($companies as $company)
                                    <option value="{{ $company->id }}">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="edit_name" x-model="editData.name"
                        ...
                        <div>
                            ...
                            <textarea name="description" id="edit_description" rows="3" x-model="editData.description"
                                placeholder="Deskripsi..."
                                class="w-full px-3 py-2 text-xs rounded-xl bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-800 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition"></textarea>
                        </div>
    ...
    <div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto"
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="showDeleteModal" x-transition:enter="ease-out duration-300"
                ...
                x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
                x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                class="inline-block align-bottom bg-white dark:bg-slate-900 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md w-full border border-gray-200 dark:border-slate-800">

                <div class="p-6">
                    <div class="flex items-center gap-4 pb-4 border-b border-gray-100 dark:border-slate-800">
                        <div
                            class="w-10 h-10 rounded-full bg-rose-100 dark:bg-rose-900/30 flex items-center justify-center shrink-0 text-rose-600 dark:text-rose-400">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500 dark:text-slate-400 mt-0.5">Tindakan ini permanen dan tidak
                                dapat dibatalkan.</p>
                        </div>
                    </div>
                    ...
                    <div class="mt-4">
                        <p class="text-xs text-gray-600 dark:text-slate-300">
                            Apakah Anda yakin ingin menghapus data departemen <strong
                                class="text-gray-900 dark:text-white font-semibold"
                                x-text="deleteData.name"></strong>?
                        </p>
                    </div>

                    <form :action="'/admin/departments/' + deleteData.id" method="POST"
                        @submit="isSubmittingDelete = true" class="mt-6 flex items-center justify-end gap-3">
                        @csrf
                        @method('DELETE')
                        ...
                        <button type="submit" :disabled="isSubmittingDelete"
                            class="px-4 py-2 text-xs font-semibold text-white bg-rose-600 hover:bg-rose-500 rounded-xl shadow-md shadow-rose-500/20 transition flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed">
                            <svg x-show="isSubmittingDelete" class="animate-spin w-3.5 h-3.5 text-white"
