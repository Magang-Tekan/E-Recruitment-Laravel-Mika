<div class="space-y-6" x-data="{ 
    ...
    openEditModal(degree) {
        this.editData = {
            id: degree.id,
            name: degree.name || '',
            rank: degree.rank !== undefined ? degree.rank : 0
        };
    ...
    <div class="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 overflow-hidden shadow-sm rounded-2xl p-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h3 class="text-lg font-bold text-gray-900 dark:text-white">Daftar Tingkat Pendidikan</h3>
                <p class="text-xs text-gray-500 dark:text-slate-400">Kelola data master tingkat/jenjang pendidikan terdaftar.</p>
            </div>
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
                ...
                <button @click="showCreateModal = true" class="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white shadow-md shadow-emerald-600/20 dark:bg-[#93F514] dark:hover:bg-[#82dc12] dark:text-black dark:shadow-[#93F514]/20 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 w-full sm:w-auto shrink-0 active:scale-95 cursor-pointer">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <table class="w-full text-left border-collapse text-xs">
                <thead>
                    <tr class="border-b border-gray-200 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-800/50 text-gray-500 dark:text-slate-400 uppercase tracking-wider font-semibold">
                        ...
                        <th class="px-6 py-4">Nama Tingkat Pendidikan</th>
                        <th class="px-6 py-4">Urutan (Rank)</th>
                            ...
                            <td class="px-6 py-4 font-bold text-gray-900 dark:text-white">
                                <div class="flex items-center gap-2.5">
                                    <div class="w-8 h-8 rounded-lg bg-indigo-50 dark:bg-indigo-950/50 border border-indigo-200 dark:border-indigo-800 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-semibold shrink-0">
                                        {{ strtoupper(substr($degree->name, 0, 1)) }}
                                    </div>
                                    <span>{{ $degree->name }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-gray-100 dark:bg-slate-800 text-gray-800 dark:text-slate-200 border border-gray-200 dark:border-slate-700">
                                    Rank {{ $degree->rank }}
                                </span>
                            </td>
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-gray-400 dark:text-slate-500">
                                <div class="flex flex-col items-center justify-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Belum ada data tingkat pendidikan</span>
                                    <span class="text-xs">Klik tombol "Tambah Tingkat Pendidikan" untuk menambahkan data baru.</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
                ...
                <div class="p-6">
                    <div class="flex items-center justify-between pb-4 border-b border-gray-100 dark:border-slate-800">
                        <h3 class="text-base font-bold text-gray-900 dark:text-white">
                            Tambah Tingkat Pendidikan Baru
                        </h3>
                    ...
                    <form action="{{ route('admin.degree.store') }}" method="POST" @submit="isSubmittingCreate = true" class="mt-4 space-y-4">
                        @csrf

                        <div>
                            <label for="name" class="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1">
                                Nama Tingkat Pendidikan <span class="text-rose-500">*</span>
                            </label>
                            <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Contoh: SMA/SMK / D3 / S1" class="w-full px-3 py-2 text-xs rounded-xl bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-800 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition">
                            @error('name')
                        ...
                        <div>
                            <label for="rank" class="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1">
                                Urutan Tingkatan (Rank)
                            </label>
                            <input type="number" min="0" name="rank" id="rank" value="{{ old('rank', '1') }}" placeholder="Contoh: 1, 2, 3" class="w-full px-3 py-2 text-xs rounded-xl bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-800 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition">
                            <span class="text-[11px] text-gray-400">Angka ini digunakan untuk urutan jenjang pendidikan dari yang terendah.</span>
                            @error('rank')
                                <p class="mt-1 text-[11px] text-rose-500">{{ $message }}</p>
                            @enderror
                        </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="showEditModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block align-bottom bg-white dark:bg-slate-900 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full border border-gray-200 dark:border-slate-800">
                ...
                <div class="p-6">
                    ...
                    <form :action="'{{ url('admin/degrees') }}/' + editData.id" method="POST" @submit="isSubmittingEdit = true" class="mt-4 space-y-4">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="edit_name" x-model="editData.name" required placeholder="Contoh: S1" class="w-full px-3 py-2 text-xs rounded-xl bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-800 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition">
                            @error('name')
                                <p class="mt-1 text-[11px] text-rose-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            <label for="edit_rank" class="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1">
                                Urutan Tingkatan (Rank)
                            </label>
                            <input type="number" min="0" name="rank" id="edit_rank" x-model="editData.rank" placeholder="Contoh: 1, 2, 3" class="w-full px-3 py-2 text-xs rounded-xl bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-800 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition">
                            @error('rank')
                                <p class="mt-1 text-[11px] text-rose-500">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500 dark:text-slate-400 mt-1">
                                Apakah Anda yakin ingin menghapus tingkat pendidikan <span class="font-bold text-gray-800 dark:text-gray-200" x-text="deleteData.name"></span>? Tindakan ini tidak dapat dibatalkan.
                            </p>
                        </div>
                    </div>

                    <form :action="'{{ url('admin/degrees') }}/' + deleteData.id" method="POST" @submit="isSubmittingDelete = true" class="mt-6 flex items-center justify-end gap-3">
                        @csrf
                        @method('DELETE')
